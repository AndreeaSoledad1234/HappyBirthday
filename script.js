...
async function startIntroVideo() {
    ...
    try {
        ...
        console.error("Videoclipul intro nu poate porni:", error);
    }
}

if (playVideoButton) {
    playVideoButton.onclick = startIntroVideo;
}

introVideo.onclick = startIntroVideo;
...
function updateHours() {
    ...
    const eventDate = new Date(2026, 6, 24 ); // 24 iulie 2026
...
memoryButton.onclick = () => {
    ...
    memoryVideo.play().catch(error => {
        console.error("Videoclipul final nu poate porni:", error);
    });
};
